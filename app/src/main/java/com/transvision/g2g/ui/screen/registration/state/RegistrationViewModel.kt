...
class RegistrationViewModel : ViewModel() {
    ...
    fun onUiEvent(registrationUiEvent: RegistrationUiEvent) {
        when (registrationUiEvent) {
            ...
            is RegistrationUiEvent.ConfirmDesignationNameChanged -> {
                registrationState.value = registrationState.value.copy(
                    designationName = registrationUiEvent.inputValue,
                    errorState = registrationState.value.errorState.copy(
                        designationNameErrorState = when {

                            // Empty state of confirm password
                            registrationUiEvent.inputValue.trim().isEmpty() -> {
                                designationNameErrorState
                            }
    ...
    private fun validateInputs(): Boolean {
        ...
        val designationNameString = registrationState.value.designationName.trim()
        ...
        return when {
            ...
            designationNameString.isEmpty() -> {
                registrationState.value = registrationState.value.copy(
                    errorState = RegistrationErrorState(
                        designationNameErrorState = designationNameErrorState
                    )
                )
                false
            }
